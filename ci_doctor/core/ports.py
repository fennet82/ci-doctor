...
class LLMClient(ABC):
    ...
    Every backend is prompt-and-parse: correctness is enforced by validation,
    not by the endpoint, because none of the endpoints we target enforce a
    schema server-side. Concretely, every implementation
    (`llm/backends.py::PydanticAILLMClient`) realizes this via Pydantic AI's
    `PromptedOutput`, which renders the schema into the model request itself
    rather than `llm/report.py` embedding it in the prompt text by hand.
    """
