...
def _fake_tty(monkeypatch, is_tty):
    """Point the CLI's stdout at a stand-in with a fixed isatty()."""
    import types

    monkeypatch.setattr(sys, "stdout", types.SimpleNamespace(isatty=lambda: is_tty))


def _clear_ci(monkeypatch):
    """Remove every CI marker so auto-detection sees a plain shell."""
    for var in ("CI", "GITHUB_ACTIONS", "GITLAB_CI"):
        monkeypatch.delenv(var, raising=False)


def test_in_ci_reads_the_usual_markers(monkeypatch):
    """`CI` alone (set by GitHub and GitLab) flips the detector."""
    _clear_ci(monkeypatch)
    assert cli._in_ci() is False
    monkeypatch.setenv("CI", "true")
    assert cli._in_ci() is True


def test_interactive_auto_on_a_bare_tty(monkeypatch):
    """A real terminal outside CI is interactive; a pipe is not."""
    _clear_ci(monkeypatch)
    _fake_tty(monkeypatch, True)
    assert cli._is_interactive(cli.OutputFormat.TERMINAL, None) is True
    _fake_tty(monkeypatch, False)
    assert cli._is_interactive(cli.OutputFormat.TERMINAL, None) is False


def test_interactive_off_in_ci_even_on_a_tty(monkeypatch):
    """CI is never interactive — nobody is there to answer the menu."""
    _clear_ci(monkeypatch)
...
def test_json_is_never_interactive(monkeypatch):
    """A machine payload stays non-interactive, flag or not."""
    _clear_ci(monkeypatch)
    _fake_tty(monkeypatch, True)
    assert cli._is_interactive(cli.OutputFormat.JSON, None) is False
    assert cli._is_interactive(cli.OutputFormat.JSON, True) is False


def test_interactive_flag_overrides_detection(monkeypatch):
    """`--interactive/--no-interactive` wins over the environment."""
    monkeypatch.setenv("CI", "true")
    _fake_tty(monkeypatch, False)
    assert cli._is_interactive(cli.OutputFormat.TERMINAL, True) is True
    _clear_ci(monkeypatch)
    _fake_tty(monkeypatch, True)
    assert cli._is_interactive(cli.OutputFormat.TERMINAL, False) is False
